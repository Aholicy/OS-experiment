...
void ProgramManager::release_zombie_process() {
     for (int i = 0; i < MAX_PROGRAM_AMOUNT; ++i) {
         if (PCB_SET_STATUS[i]) {
            if (((PCB *)((int)PCB_SET + PCB_SIZE * i))->pageDirectoryAddress)
               if (((PCB *)((int)PCB_SET + PCB_SIZE * i))->status == DEAD) {
                  int fid = ((PCB *)((int)PCB_SET + PCB_SIZE * i))->parentPid;
                  PCB *process = findProgramByPid(fid);
                  if (process->status == DEAD || !process) {
            	      printf("zombie process pid: %d , system released its PCB\n",
                     ((PCB *)((int)PCB_SET + PCB_SIZE * i))->pid);
            	      releasePCB((PCB *)((int)PCB_SET + PCB_SIZE * i));
          	   }
        	}
             }
         }
